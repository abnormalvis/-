...
xdata SystemEvent event_queue[EVENT_QUEUE_SIZE];
volatile unsigned char event_head = 0, event_tail = 0;
...
{
    handle_exit_state(current_state); // 清理旧状态
    current_state = new_state;
    handle_enter_state(new_state); // 初始化新状态
}
void enqueue_event(SystemEvent evt)
{
    if ((event_head + 1) % EVENT_QUEUE_SIZE != event_tail)
    {
        event_queue[event_head++] = evt;
        event_head %= EVENT_QUEUE_SIZE;
    }
}
